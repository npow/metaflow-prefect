...
def _build_header(
    spec: FlowSpec,
    cfg: PrefectFlowConfig,
    cmd_templates: dict[str, tuple[str, ...]],
) -> str:
    """Return the compile-time constants block for the generated file."""
    lines = [
        "# Generated by metaflow-prefect on %s" % datetime.now().isoformat(timespec="seconds"),
        "# Metaflow flow: %s" % spec.name,
        "# Do not edit this file — regenerate with: python %s prefect compile <output.py>" % cfg.flow_file,
        "",
        "from __future__ import annotations",
        "",
        "import json",
...
def _build_task(step: StepSpec, spec: FlowSpec) -> str:
    ...
    # Precompute once — both _task_signature and _task_body_lines need this set.
    foreach_body = _foreach_body_steps(spec)
    body = _task_body_lines(step, foreach_body)
    indented_body = [_INDENT + line if line else "" for line in body]
    return "\n".join([
        _task_decorator(step),
        "def %s(%s) -> %s:" % (
            _task_fn(step.name),
            _task_signature(step, foreach_body),
...
def _task_decorator(step: StepSpec) -> str:
    """Return the ``@task(...)`` decorator line for one step."""
...
def _task_signature(step: StepSpec, foreach_body: set[str]) -> str:
...
def _task_body_lines(step: StepSpec, foreach_body: set[str]) -> list[str]:
    """Return the unindented body lines for a @task function."""
    is_start = step.name == "start"
    is_foreach_body = step.name in foreach_body
...
def _input_paths_line(step: StepSpec) -> str:
...
def _flow_decorator(spec: FlowSpec, cfg: PrefectFlowConfig) -> str:
    """Return the ``@flow(...)`` decorator line."""
...
def _task_fn(step_name: str) -> str:
    """Return the Python function name for the @task of a given step."""
